...
def perform_series_run(rate_constants,numberOfSites):
...
res = opt.brute(perform_series_run,ranges=ranges,args=[10],Ns=5,finish=None)#,finish=opt.fmin)
ranges=[(res[0]-.5*res[0],res[0]+.5*res[0]),(res[1]-.5*res[1],res[1]+.5*res[1])]
res = opt.brute(perform_series_run,ranges=ranges,args=[100],Ns=5,finish=None)
ranges=[(res[0]-.25*res[0],res[0]+.25*res[0]),(res[1]-.25*res[1],res[1]+.25*res[1])]
res = opt.brute(perform_series_run,ranges=ranges,args=[200],Ns=5,finish=None)
final_output = perform_series_run(res,1000)
# while objective_function > 1:
#     x_a = np.array([x_n[0]+,x_n[1]])
#     point1 = perform_series_run(x0[0])
run_time = time.time()-start_t
print 'finished. total run time = ' + str(run_time)