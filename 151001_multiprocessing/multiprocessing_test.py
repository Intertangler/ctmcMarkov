...
sites = 16
occupancy = np.array([0,0,0])
def accumulate(occupancy):
    occupancy = occupancy + np.array([0,0,1])
    return

if __name__ == '__main__':
    jobs = []
    for i in range(4):
        p = multiprocessing.Process(target=accumulate(occupancy))
        jobs.append(p)
        p.start()
...
#     for site in range(0, numberOfSites/4):
